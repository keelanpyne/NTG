CREATE OR ALTER VIEW analytics.us_Jobs AS
WITH Jobs_AR AS (
    SELECT
    ...
    FROM cleaned.us_AccTransactionLines as A
    LEFT JOIN cleaned.us_JobCharge as B ON UPPER(A.AL_PK) = UPPER(B.JR_AL_ARLine)
    LEFT JOIN cleaned.us_JobChargeRevRecognition as C on UPPER(C.D3_JH) = UPPER(B.JR_JH) AND UPPER(C.D3_RecognitionType) = UPPER(A.AL_RevRecognitionType)
    LEFT JOIN cleaned.us_JobHeader as D ON UPPER(D.JH_PK) = UPPER(B.JR_JH)
    LEFT JOIN cleaned.us_JobShipment AS F ON UPPER(F.JS_PK) = UPPER(D.JS)),
    ...
    FROM cleaned.us_AccTransactionLines as A
    LEFT JOIN cleaned.us_JobCharge as B ON UPPER(A.AL_PK) = UPPER(B.JR_AL_APLine)
    LEFT JOIN cleaned.us_JobChargeRevRecognition as C on UPPER(C.D3_JH) = UPPER(B.JR_JH) AND UPPER(C.D3_RecognitionType) = UPPER(A.AL_RevRecognitionType))
